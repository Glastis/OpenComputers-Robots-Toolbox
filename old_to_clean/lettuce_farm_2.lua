...
function free_crafting_table()
	...
	while slot <= 11 do
		if slot ~= 4 and slot ~= 8 then
			while component.inventory_controller.getStackInInternalSlot(slot) and not push_item_after_slot(slot) do
        print("Inventory looks full. Please help me.")
        os.sleep(120)
			end
		end
		slot = slot + 1
...
-- USING GLOBAL VARIABLE SLEEP_TIME

function get_free_slot_in_chest(chest_side, item_to_stack, max_stacksize)
	local slot
	local inv_size
	local data
	local reason
	...
	slot = 1
	if not item_to_stack then
		item_to_stack = "none"
	end
	if not max_stacksize then
		max_stacksize = 0
	end
	...
	while slot < inv_size do
		...
		if not data or (data.name == item_to_stack and data.size < max_stacksize) then
			return slot
		end
		slot = slot + 1
	end
	if SLEEP_TIME then
		print("Chest seems full. Waiting " .. SLEEP_TIME .. " secs.")
		os.sleep(SLEEP_TIME)
		return get_free_slot_in_chest(chest_side, item_to_stack, max_stacksize)
	end
	return false
end
...
function dump_slot_to_chest(chest_side, from_slot, amount, to_slot)
	...
	if not to_slot then
		to_slot = get_free_slot_in_chest(chest_side, data.name, 32)
		if not to_slot then
			return false
		end
	end
	...
	if not amount then
		retval = component.inventory_controller.dropIntoSlot(chest_side, to_slot)
	else
		retval = component.inventory_controller.dropIntoSlot(chest_side, to_slot, amount)
	end
	data = component.inventory_controller.getStackInInternalSlot(from_slot)
	if not data then
		return retval
	end
	return dump_slot_to_chest(chest_side, from_slot, amount, false)
end
...
function craft_and_store_food()
	local lettuce_side = side.up
	local stock_side = side.front

	if not can_drop_in_chest() then
		print("No chest found")
		return false
	end
	drop_item_to_chest(lettuce_side, "harvestcraft:lettuceItem")
	if not free_crafting_table() or not place_item_for_craft("harvestcraft:potItem", 1, 0) then
		return false
	end
	get_item_from_chest(lettuce_side, "harvestcraft:lettuceItem", nil, 21)
	while place_item_for_craft("harvestcraft:lettuceItem", 2, 0, 21) do
		component.crafting.craft()
		select_empty_slot()
		drop_item_to_chest(stock_side, "harvestcraft:stockItem")
		get_item_from_chest(lettuce_side, "harvestcraft:lettuceItem", nil, 21)
	end
	return true
end
